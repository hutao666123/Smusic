...
class PlaylistManager {
    ...
    try {
      ...
      if (!result.success) {
        ...
        if (result.error && result.error.code === 'JSON_PARSE_ERROR') {
          console.error('favorites.json 文件损坏，创建新文件:', result.error.message)
  ...
  async getCustomPlaylists() {
    try {
      ...
      if (!result.success) {
        ...
        // 如果是 JSON 解析错误，记录错误并创建新文件
        if (result.error && result.error.code === 'JSON_PARSE_ERROR') {
          console.error('custom-playlists.json 文件损坏，创建新文件:', result.error.message)
  ...
  async collectOnlinePlaylist(playlist) {
    try {
      ...
      // 检查是否已收藏
      const exists = data.playlists.some(p => p.id === playlist.id)
      if (exists) {
        return {
          success: true,
          data: data.playlists,
      ...
      // 保存到文件
      const writeResult = await this.fileManager.writeJSON(this.COLLECTED_PLAYLISTS_FILE, data)
      if (!writeResult.success) {
        return writeResult
      }
  ...
  async getCollectedPlaylists() {
    try {
      ...
      if (!result.success) {
        ...
        // 如果是 JSON 解析错误，记录错误并创建新文件
        if (result.error && result.error.code === 'JSON_PARSE_ERROR') {
          console.error('collected-playlists.json 文件损坏，创建新文件:', result.error.message)
  ...
  async getDownloads() {
    try {
      ...
      if (!result.success) {
        ...
        // 如果是 JSON 解析错误，记录错误并创建新文件
        if (result.error && result.error.code === 'JSON_PARSE_ERROR') {
          console.error('downloads.json 文件损坏，创建新文件:', result.error.message)
  ...
  async getLocalSongPath(songId) {
    try {
      ...
      console.log('getLocalSongPath - getDownloads 结果:', result)
      ...
      console.log('getLocalSongPath - result.data:', result.data)
      console.log('getLocalSongPath - result.data.songs:', result.data.songs)
      
      // 转换为字符串进行比较，避免类型不匹配
      const songIdStr = String(songId)
      const song = result.data.songs.find(s => String(s.id) === songIdStr)
      
      console.log('getLocalSongPath - 找到的歌曲:', song)
