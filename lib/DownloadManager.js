...
class DownloadManager {
  ...
  async _executeDownload(task) {
    return new Promise((resolve, reject) => {
      ...
      const sanitizedId = this._sanitizeFileName(task.songId)
      ...
      const request = protocol.get(task.songUrl, (response) => {
        ...
        fileStream.on('finish', async () => {
          ...
          // 添加到"已下载"歌单
          try {
            console.log('下载完成，准备添加到已下载歌单')
            console.log('task.metadata:', task.metadata)
            console.log('filePath:', filePath)
            console.log('task.fileSize:', task.fileSize)
            console.log('task.quality:', task.quality)
            ...
            // 记录错误但不影响下载状态
            console.error('添加到已下载歌单失败:', error)
          }
