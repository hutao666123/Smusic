...
class FileManager {
  constructor(userDataPath, options = {}) {
    ...
    // 支持自定义下载目录
    const customDownloadPath = options.customDownloadPath
    this.downloadsDir = customDownloadPath || path.join(userDataPath, 'downloads')
   ...
   */
  updateDownloadPath(newDownloadPath) {
    this.downloadsDir = newDownloadPath
    this.songsDir = path.join(this.downloadsDir, 'songs')
    this.coversDir = path.join(this.downloadsDir, 'covers')
    this.lyricsDir = path.join(this.downloadsDir, 'lyrics')
  }
